...
interface DownloadModalProps {
  ...
  generateFullPageHtml?: (item: any, lang: DownloadLanguage, design: DownloadDesignConfig) => string | string[];
  generateSummaryPageHtml?: (items: any[], lang: DownloadLanguage, design: DownloadDesignConfig) => string | string[]; 
        ...
        try {
          if (downloadFormat === 'pdf-full' && generateFullPageHtml) {
              ...
              for (let i = 0; i < selectedItems.length; i++) {
                  ...
                  const itemHtmlOrPages = generateFullPageHtml(item, downloadLanguage, designConfig);
                  const itemPages = Array.isArray(itemHtmlOrPages) ? itemHtmlOrPages : [itemHtmlOrPages];

                  for (let j = 0; j < itemPages.length; j++) {
                    tempContainer.innerHTML = itemPages[j];
