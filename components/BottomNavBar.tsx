...
const BottomNavBar: React.FC<BottomNavBarProps> = ({ t, currentPage, setCurrentPage, navAnimation, showLabels }) => {
  ...
  return (
    <div className={`xl:hidden fixed bottom-0 left-0 w-full z-50 transition-transform duration-300 ${isCollapsed ? 'translate-y-full' : 'translate-y-0'}`}>
      <div className="relative w-full bg-[var(--bg-secondary)] h-16 rounded-t-2xl shadow-[0_-5px_15px_rgba(0,0,0,0.05)] border-t border-[var(--border-secondary)]">
        
        {/* Moving Indicator */}
        <div 
            className="absolute -top-6 h-12 w-12 bg-[var(--accent-primary)] rounded-full transition-all duration-300 ease-out flex items-center justify-center shadow-lg z-20"
            style={{ 
                left: `calc((100% / ${itemCount}) * ${activeIndex} + (100% / ${itemCount} / 2) - 1.5rem)` 
            }}
        >
            {/* Inverted Curves (Fillets) */}
            <span 
                className="absolute top-[50%] -left-[18px] w-[18px] h-[18px] bg-transparent rounded-tr-[18px]"
                style={{ boxShadow: '5px -5px 0 0 var(--bg-secondary)' }}
            ></span>
            <span 
                className="absolute top-[50%] -right-[18px] w-[18px] h-[18px] bg-transparent rounded-tl-[18px]"
                style={{ boxShadow: '-5px -5px 0 0 var(--bg-secondary)' }}
            ></span>
            
            {/* Active Icon (Duplicate for smooth transition) */}
            ...
            {navItems.map((item, index) => {
                ...
                return (
                    <button 
                        ...
                        className="flex-1 relative flex flex-col items-center justify-center z-10 focus:outline-none"
                    >
                        <span 
                            className={`transition-all duration-300 absolute ${isActive ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0 text-[var(--text-secondary)]'}`}
                        >
                            {item.icon}
                        </span>
                        ...
                        {showLabels && (
                            <span 
                                className={`text-[10px] font-medium mt-8 transition-all duration-300 absolute ${isActive ? 'opacity-100 translate-y-0 text-[var(--accent-primary)]' : 'opacity-0 translate-y-2'}`}
                            >
