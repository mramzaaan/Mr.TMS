  ...
  return (
    <div className="p-6 bg-[var(--bg-secondary)] rounded-lg shadow-md border border-[var(--border-primary)] space-y-6">
        ...
        <div className="space-y-3">
            <div className="text-xs text-[var(--text-secondary)] italic mb-2">Uncheck the box to mark a day as a Holiday/Inactive (e.g. Sunday or special holidays). This affects workload calculations.</div>
            {allDays.map(day => {
                const config = localDaysConfig[day] || { active: true, periodCount: 8 };
                return (
                    <div key={day} className={`flex items-center justify-between p-3 rounded-lg border transition-colors ${config.active ? 'bg-[var(--bg-tertiary)] border-[var(--border-secondary)]' : 'bg-[var(--bg-primary)] border-transparent opacity-60'}`}>
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                                <span className={`font-semibold text-[var(--text-primary)] ${!config.active ? 'line-through text-[var(--text-secondary)]' : ''}`}>
                                    {t[day.toLowerCase()]}
                                </span>
                                {!config.active && <span className="text-[10px] text-red-500 font-bold uppercase tracking-wider">Holiday / Off</span>}
                            </div>
                        </div>
                    ...
                    if (item.type === 'period') {
                        return (
                            <div key={`p-${item.index}`} className="flex items-center gap-3 bg-[var(--bg-secondary)] p-2 rounded-md border border-[var(--border-secondary)]">
                                ...
                                <input 
                                    ...
                                    className="text-sm font-semibold w-24 bg-[var(--bg-tertiary)] text-[var(--text-primary)] border border-[var(--border-secondary)] rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-[var(--accent-primary)]"
                        ...
                        return (
                            <div key="assembly" className="flex items-center gap-3 bg-indigo-50 dark:bg-indigo-900/20 p-2 rounded-md border border-indigo-200 dark:border-indigo-800">
                                <span className="text-sm font-bold w-24 text-indigo-700 dark:text-indigo-300 flex items-center gap-1 pl-2">
                                ...
                                <input 
                                    ...
                                    value={item.data.start} 
                                    onChange={(e) => updateScheduleData(flatIndex, 'start', e.target.value)}
                                    className="bg-[var(--bg-secondary)] border border-indigo-300 dark:border-indigo-700 rounded px-2 py-1 text-sm text-[var(--text-primary)] focus:outline-none focus:ring-1 focus:ring-indigo-500"
                                />
                        ...
                        return (
                            <div key={`b-${item.index}`} className="flex items-center gap-3 bg-orange-50 dark:bg-orange-900/20 p-2 rounded-md border border-orange-200 dark:border-orange-800">
                                <span className="text-sm font-bold w-24 text-orange-700 dark:text-orange-300 flex items-center gap-1 pl-2">
                                ...
                                <input 
                                    ...
                                    onChange={(e) => updateScheduleData(flatIndex, 'start', e.target.value)}
                                    className="bg-[var(--bg-secondary)] border border-orange-300 dark:border-orange-700 rounded px-2 py-1 text-sm text-[var(--text-primary)] focus:outline-none focus:ring-1 focus:ring-orange-500"
                                />
